...
from numpy import abs, append, arange, insert, linspace, log10, round, zeros, shape
...
'''
f1, f2 = 40, 18000
melmat, (melfreq, fftfreq) = compute_melmat(30, f1, f2, num_fft_bands=22051, sample_rate=44100)
print(shape(melmat))
print(shape(melfreq))
print(melfreq)
print(shape(fftfreq))
...
for i in range(len(melmat)):
    print('filter ', i)
    found_start = False
    found_end = False
    for j in range(len(melmat[i])):
        if melmat[i][j] > 0 and not found_start:
            found_start = True
            print('start ', j, ' val ', melmat[i][j])
        if j < len(melmat[i]) - 1 and melmat[i][j] > 0 and melmat[i][j + 1] == 0 and not found_end:
            found_end = True
            print('end ', j, ' val ', melmat[i][j])
        if j-1 > 0 and j + 1 < len(melmat[i]) and melmat[i][j-1] < melmat[i][j] and melmat[i][j+1] < melmat[i][j]:
            print('peak ', j, ' val ', melmat[i][j])
